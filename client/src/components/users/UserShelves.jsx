...
export default function UserShelves() {
  const { data: reading, isLoading: readingIsLoading } = useGetUserShelvesByNameQuery("Currently Reading");
  const { data: read, isLoading: readIsLoading } = useGetUserShelvesByNameQuery("Read");
  const { data: wantToRead, isLoading: wantToReadIsLoading } = useGetUserShelvesByNameQuery("Want to Read");

  if (readingIsLoading || readIsLoading || wantToReadIsLoading) return <p>Loading forums...</p>;
