...
router.get("/:userId/user", async (req, res) => {
    try {
      const shelves = await Shelf.find({ user: req.params.userId }).populate("books", "title author");
...
router.get("/:shelfId", async (req, res) => {
  try {
    const shelf = await Shelf.findById(req.params.shelfId).populate("books", "title author");
...
router.delete("/:id", auth, async (req, res) => {
  ...
  router.post("/:shelfId/add", auth, async (req, res) => {
    try {
      const shelf = await Shelf.findOne({ _id: req.params.shelfId, user: req.user.userId });
      if (!shelf) return res.status(404).json({ message: "Shelf not found" });
